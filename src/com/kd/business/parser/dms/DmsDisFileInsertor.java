 ...
 *
 */
public class DmsDisFileInsertor implements DmsFileInsertorInterface {
	@Autowired
	private SystemService systemService;

	private static final Logger logger = Logger.getLogger(DmsDisFileInsertor.class);
	
	@Override
	public boolean isMyFile(String fileName) {
		boolean isMyFile = false;
		if (fileName != null && fileName.endsWith("_dsc.xml")) {
			isMyFile = true;
		}
		return isMyFile;
	}
	...
	public void insertToDB(File f, String bakFolder) {
		try {
			...
			saxp.parse(FileUtil.readFileInputSource(f, "GBK"), new DmsDisHandler(f, bakFolder, sqlList,systemService));
			if(sqlList.size()>0){				
	...
	class DmsDisHandler extends DmsSaxHandler {
		public DmsDisHandler() {
		}

		public DmsDisHandler(File file, String bakFolder,List<String> sqlList,SystemService systemService) {
...
//					attributes.getValue("remotetype"));
				String sql = "INSERT INTO EVALUSYSTEM.DETAIL.DMSCB(ORGAN_CODE,DV,CB,NAME,TYPE,GTIME,STATUS,FLAG,REMOTETYPE)VALUES("
						+code+",'"+attributes.getValue("dvid")+"','"+attributes.getValue("dsc")+"','"+name+"','0','"+gTime+
